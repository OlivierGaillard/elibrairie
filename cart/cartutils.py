...
def get_cart_item_of_book(cart_items, book):
    cart_items_li = [ cart_item for cart_item in cart_items if cart_item.book == book ]
    if len(cart_items_li) == 1:
        return cart_items_li[0]
    else:
        return None

def _remove_cart_item(request, book):
    """Remove the cart_item of the book."""
    cart_items = get_cart_items(request)
    if cart_items:
        cart_item_of_book = get_cart_item_of_book(cart_items, book)
        cart_item_of_book.delete()

def article_already_in_cart(cart_items, article):
    articles_pk_list = [ cart_item.book.pk for cart_item in cart_items ]
